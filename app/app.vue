...
function useAppSeo() {
  ...
  useHead({
    titleTemplate: (c) =>
      c ? `${c} - Alexander Lichter` : "Alexander Lichter - Web Engineering Consultant",
...
function useSiteNotifications() {
  ...
  function onUserSpeaksGerman(): boolean {
    ...
    const didUserSeeGermanNotificationAlready = localStorage.getItem(
      LOCALSTORAGE_KEYS.notificationGerman,
    );
