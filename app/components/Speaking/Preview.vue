...
<template>
  <div
    class="md:grid md:grid-cols-2 gap-8 bg-zinc-800 p-8 transition-all border border-transparent"
    :class="isHoveringOverLink && 'hover:border-red-500'"
  >
    <div>
      <header class="flex-grow">
        <AppLink
          class="transition-all underline decoration-transparent hover:decoration-white"
      ...
      <ul class="flex flex-col md:flex-row mt-4">
        <li v-if="talk.eventName">
          ...
          <span v-if="talk.location || talk.date" class="hidden md:inline-block" aria-hidden
            >&bull;&nbsp;</span
          >
        </li>
