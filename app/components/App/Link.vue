...
const isDomainException = computed(
  () =>
    ...
    ALLOWED_DOMAINS.some((domain) => (path.value as string).startsWith(domain)),
);
const defaultRel = computed(() =>
  isExternal.value && !isDomainException.value ? "nofollow noindex noreferrer" : null,
);
