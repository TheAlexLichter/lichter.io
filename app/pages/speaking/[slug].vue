...
const { data: page } = await useAsyncData(`speaking-${route.params.slug}`, () =>
  queryCollection("speaking").path(`/speaking/${route.params.slug}`).first(),
);
...
provide(
  "content-toc",
  computed(() => page.value?.body?.toc),
);

const { data: surround } = await useAsyncData(`speaking-surround-${route.params.slug}`, () =>
  queryCollectionItemSurroundings("speaking", `/speaking/${route.params.slug}`, {
    fields: [
      "title",
      "path",
      "description",
...
const formattedDate = computed(() =>
  page.value?.date ? formatDateStringToHumanReadable(page.value.date) : "",
);
...
const links = computed(() => {
  ...
  const twitterLink =
    page.value!.podcastUrl ??
    page.value!.videoUrl ??
    page.value!.slidesUrl ??
    linkToCurrentPage.value;
  const rawDiscussOnTwitter = `https://twitter.com/search?q=${twitterLink}`;

  const editOnGitHub = `https://github.com/manniL/lichter.io-playground/edit/main/content/${page.value!.stem}${page.value!.extension}`;
  return {
    shareOnTwitter,
...
function isTalkOrPodcast(entry?: { path?: string } | null): boolean {
...
<template>
  <div>
      ...
      <div class="flex flex-col md:flex-row gap-4 md:gap-0 justify-between mt-8">
        <ul class="flex flex-col md:flex-row">
          <li v-if="page!.eventName">
            ...
            <span v-if="page!.location || page!.date" class="hidden md:inline-block" aria-hidden
              >&bull;&nbsp;</span
            >
          </li>
      ...
      <div class="mt-16">
        <ul class="flex flex-row gap-4 md:gap-8">
          <li v-if="page!.slidesUrl">
            <AppLink
              class="border-b-4 border-white/75 hover:border-white transition-all pr-1 pb-1"
              :to="page!.slidesUrl"
            >
    ...
    <AppSection class="justify-center bg-zinc-900 pb-8">
      <div>
        <ContentRenderer class="prose md:prose-lg lg:prose-xl pt-4" :value="page!" />
      </div>
      ...
      >
        <div class="shrink-0">
          <img
            class="md:w-48 md:h-48 rounded-full mx-auto"
            width="192"
            height="192"
            src="/img/me@2x.jpg"
