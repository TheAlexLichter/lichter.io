...
const { data: page } = await useAsyncData(`speaking-${route.params.slug}`, () => queryCollection('speaking').path(`/speaking/${route.params.slug}`).first())

if (!page.value) {
  throw createError({ statusCode: 404, fatal: true })
}

provide('content-toc', computed(() => page.value?.body?.toc))

const { data: surround } = await useAsyncData(`speaking-surround-${route.params.slug}`, () => queryCollectionItemSurroundings('speaking', `/speaking/${route.params.slug}`, { fields: ['title', 'path', 'description', 'date', 'eventName', 'eventUrl', 'location', 'slidesUrl', 'videoUrl', 'podcastUrl', 'type', 'topics'] }))

const prev = computed(() => surround.value?.[0])
const next = computed(() => surround.value?.[1])

const formattedDate = computed(() => page.value?.date ? formatDateStringToHumanReadable(page.value.date) : '')
