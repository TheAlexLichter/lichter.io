...
const { data: workshops } = await useAsyncData(`topic-workshops-${slug}`, () =>
  queryCollection("workshops").where("topics", "LIKE", `%${slug}%`).all(),
);

const { data: articles } = await useAsyncData(`topic-articles-${slug}`, () =>
...
const { data: talks } = await useAsyncData(`topic-speaking-${slug}`, () =>
  queryCollection("speaking").where("topics", "LIKE", `%${slug}%`).order("date", "DESC").all(),
);
...
<template>
  <div>
    <AppSection class="bg-gradient-to-b !pb-4">
      ...
      <AppParagraph class="mt-4" look="heading" tag="h1"> Topic: {{ topicName }} </AppParagraph>
    </AppSection>
    <AppSection class="justify-center pb-8">
      <template v-if="workshops?.length">
        <AppParagraph class="pt-16 !text-4xl" look="heading" tag="h2"> Workshops </AppParagraph>
      ...
      <template v-if="articles?.length">
        <AppParagraph class="pt-16 !text-4xl" look="heading" tag="h2"> Articles </AppParagraph>
