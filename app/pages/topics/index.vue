...
const title = "Topics";
const description =
  "Being it talks, workshops, panels, podcasts or blog posts, here you can find all my content sorted by topic.";
...
const { data: topics } = await useAsyncData("all-topics", async () => {
  ...
  return [...counts.entries()]
    .map(([slug, count]) => ({
      ...
      count,
      size: maxCount > 1 ? Math.min(3, Math.floor((Math.log(count) / Math.log(maxCount)) * 4)) : 1,
    }))
...
<template>
  <AppSection>
    ...
    <AppParagraph class="max-w-3xl mt-8" look="subParagraph">
      Below you can find a list of my talks, podcast appearances, given workshops and panels. All,
      sorted by topics that might interest you.
    </AppParagraph>
